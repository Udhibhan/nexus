...
export default function Dashboard({
  ...
  useEffect(() => {
    ...
    import('mqtt').then(({ connect }) => {
      ...
      c.on('message', (_t: string, buf: Uint8Array) => {
        try {
          onEvent(JSON.parse(new TextDecoder().decode(buf)))
        } catch {
          // ignore bad payloads
        }
      })
  ...
  async function onEvent(data: { event: string }) {
    ...
    if (event === 'arrived_home') {
      await supabase
        .from('bot_state')
        .update({ status: 'idle', delivery_id: null })
        .eq('id', 1)
  ...
  async function callBot() {
    ...
    if (del) {
      ...
      await supabase
        .from('bot_state')
        .update({ status: 'going_pickup', delivery_id: del.id })
        .eq('id', 1)
    }
  ...
  const dispatchableLocs = locations.filter(
    (l) => !l.is_home && l.id !== myLoc?.id
  )
    ...
    <div style={{ minHeight: '100vh', padding: '24px', maxWidth: '860px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '28px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <button className="btn" style={{ fontSize: '11px', padding: '6px 14px' }} onClick={logout}>
            Logout
          </button>
        </div>
      </div>
          ...
          <span
            style={{
              ...
              color:
                botStatus === 'idle'
                  ? 'var(--muted)'
                  : botStatus === 'at_pickup' || botStatus === 'at_delivery'
                    ? 'var(--green)'
                    : 'var(--amber)',
            }}
          >
        ...
        {delivery && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px' }}>
            {[
              {
                label: 'From',
                val: locations.find((l) => l.id === delivery.pickup_location_id)?.label || '—',
              },
              {
                label: 'To',
                val: locations.find((l) => l.id === delivery.delivery_location_id)?.label || '—',
              },
              {
                label: 'Passcode',
                val: amRecipient ? delivery.passcode || '—' : delivery.passcode ? '✦ ✦ ✦ ✦' : '—',
              },
            ].map(({ label, val }) => (
              <div
                key={label}
              ...
              >
                ...
                <div
                  style={{
                    fontFamily: 'JetBrains Mono, monospace',
                    fontSize: label === 'Passcode' ? '18px' : '12px',
                    color: label === 'Passcode' && amRecipient ? 'var(--amber)' : 'var(--text)',
                    marginTop: '3px',
                    letterSpacing: label === 'Passcode' ? '4px' : 'normal',
                  }}
                >
                  {val}
                </div>
      ...
      {/* Action panel */}
      <div className="card" style={{ padding: '20px', marginBottom: '16px' }}>
        <span className="label" style={{ display: 'block', marginBottom: '14px' }}>
          Actions
        </span>
        ...
        {(botStatus === 'in_transit' || botStatus === 'at_delivery') && amRecipient && delivery?.passcode && (
          <div style={{ marginBottom: '16px' }}>
            ...
            >
              YOUR PASSCODE — show this to enter on the bot:
            </div>
            <div
              style={{
                fontFamily: 'JetBrains Mono, monospace',
        ...
        <div className="modal-overlay" onClick={() => setShowDispatch(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              <button className="btn" onClick={() => setShowDispatch(false)}>
                Cancel
              </button>
