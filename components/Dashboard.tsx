...
export default function Dashboard({ userId, profile, locations, allProfiles, initialDelivery, initialBotState }: Props) {
  ...
  useEffect(() => {
    ...
    import('mqtt').then((mod) => {
      ...
      // CRITICAL: Ensure your .env.local has wss:// and :8884/mqtt
  ...
  async function handleEvent(event: MqttStatusEvent) {
    ...
    if (event === 'arrived_location') {
      if (botStatus === 'going_pickup' || !delivery?.delivery_location_id) {
        // Arrived at pickup, waiting for the 5s servo sequence to finish
        ...
        if (amRecip) showToast('Bot arrived! Enter passcode on the physical keypad.')
      }
    } else if (event === 'load_received') {
      // Hardware finished servo open -> weigh -> close sequence
      ...
      // Recipient entered correct OTP on physical keypad
  ...
  async function startDelivery() {
    ...
    // Sends the passcode to the R4 over MQTT so it can verify the physical keypad later
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div className="card" style={{ padding: 20 }}>
            ...
            {/* Display the OTP to the recipient so they know what to type on the physical keypad */}
